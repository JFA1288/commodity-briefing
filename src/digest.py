...
def build_company_digests(company_news: dict[str, list[NewsItem]]) -> list[CompanyDigest]:
    ...
    # Include ALL configured companies, even those without news
...
def build_outlook(
    ...
    for comm_name in key_commodities:
        ...
        # World Bank / FRED data shown as wb_forecast if available
        wb_text = ""
        if fund and fund.source in ("World Bank", "FRED"):
            wb_text = fund.balance_read
        elif not fund:
            # Try alternate names: "Crude Oil" matches "Brent Crude"/"WTI Crude" FRED items
